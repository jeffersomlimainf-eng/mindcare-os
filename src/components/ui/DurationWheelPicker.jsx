...
const Wheel = ({ items, value, onChange }) => {
    ...
    const [selectedIndex, setSelectedIndex] = useState(() => {
        const idx = items.indexOf(parseInt(value));
        return idx !== -1 ? idx : 0;
    });

    useEffect(() => {
        const index = items.indexOf(parseInt(value));
        if (index === -1) return;
    ...
    return (
        <div
            ref={containerRef}
            className="h-[132px] flex-1 overflow-y-scroll snap-y snap-mandatory select-none no-scrollbar"
...
const DurationWheelPicker = ({ value, onChange }) => {
    const currentVal = parseInt(value) || 50;
    const minutes = Array.from({ length: 30 }, (_, i) => (i + 1) * 5);

    return (
        <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden w-[140px]">
            {/* Cabeçalho */}
            ...
            <div className="flex justify-center pt-1.5 pb-0">
                <span className="text-[9px] font-black text-primary/70 uppercase tracking-wider">MIN</span>
            </div>
            ...
            <div className="relative px-3 pb-0">
                ...
                <Wheel items={minutes} value={currentVal} onChange={onChange} />
            </div>
