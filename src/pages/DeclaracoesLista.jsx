...
const DeclaracoesLista = () => {
    ...
    const { declaracoes, deleteDeclaracao } = useDeclaracoes();
    ...
    const filtrados = declaracoes.filter(d => {
        const matchBusca = (d.pacienteNome || '').toLowerCase().includes(busca.toLowerCase()) ||
            (d.documentoId || '').toLowerCase().includes(busca.toLowerCase());
        ...
        matchStatus = normalizedFiltro === 'Todos' || d.status === normalizedFiltro;
        return matchBusca && matchStatus;
    });

    const totalFinalizado = declaracoes.filter(d => d.status === 'Finalizado').length;
    const totalRascunho = declaracoes.filter(d => d.status === 'Rascunho').length;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-1">
                <div>
                    <div className="flex items-center gap-2 text-primary mb-1">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest opacity-60">Comprovantes</span>
                    </div>
                    <h1 className="text-slate-900 dark:text-slate-100 text-3xl font-bold tracking-tight">Declarações</h1>
                    <p className="text-slate-500 font-medium mt-1">Emissão de comprovantes de comparecimento.</p>
                </div>
                <button
                    onClick={() => navigate('/declaracoes/novo')}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 px-1">
                {[
                    { label: 'Todos', value: declaracoes.length, icon: 'folder_open', cor: 'text-primary bg-primary/10' },
                ...
                <div className="flex-1 w-full bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                    <label className="relative flex items-center w-full">
                        ...
                        <input
                            ...
                            placeholder="Buscar declaração por paciente..."
                            value={busca}
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 dark:bg-slate-800/50">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-slate-400">Identificação</th>
                                ...
                                >
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className={`size-10 rounded-full flex items-center justify-center text-xs font-bold ${d.pacienteCor || 'bg-primary/10 text-primary'}`}>
                                                {d.pacienteIniciais || '?'}
                                            </div>
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <span className={`inline-flex px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide ${statusConfig[d.status] || 'bg-slate-100 text-slate-600'}`}>
                                            {d.status}
                                        </span>
                                    </td>
                    ...
                    declaracoes.length === 0 ? (
                        <div className="py-16 flex flex-col items-center text-center px-6">
                            <div className="size-24 rounded-3xl bg-primary/5 border border-primary/10 flex items-center justify-center mb-5">
                                <span className="material-symbols-outlined text-5xl text-primary/30">verified</span>
                            </div>
                            <h3 className="text-lg font-black text-slate-800 dark:text-white mb-2">Nenhuma declaração ainda</h3>
                            <p className="text-sm text-slate-400 max-w-sm mx-auto mb-6 leading-relaxed">
                                Declarações de comparecimento comprovam a presença do paciente nas consultas para justificativas profissionais ou escolares.
                            </p>
                            <button onClick={() => navigate('/declaracoes/novo')} className="px-6 py-3 bg-primary text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all flex items-center gap-2">
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setConfirmDelete(null)}>
                    <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-sm overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className="p-6 text-center">
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Excluir Declaração?</h3>
                            ...
                            <div className="flex gap-3">
                                ...
                                <button onClick={() => { deleteDeclaracao(confirmDelete); setConfirmDelete(null); }} className="flex-1 py-2 bg-red-500 text-white rounded-lg text-sm font-bold">Excluir</button>
                            </div>
                        </div>
                    </div>
                </div>
