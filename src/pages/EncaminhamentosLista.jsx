...
const EncaminhamentosLista = () => {
    ...
    const { encaminhamentos, deleteEncaminhamento } = useEncaminhamentos();
    ...
    const filtrados = encaminhamentos.filter(e => {
        const matchBusca = (e.pacienteNome || '').toLowerCase().includes(busca.toLowerCase()) ||
            (e.documentoId || '').toLowerCase().includes(busca.toLowerCase()) ||
            (e.especialidadeDestino || '').toLowerCase().includes(busca.toLowerCase());
        ...
        matchStatus = normalizedFiltro === 'Todos' || e.status === normalizedFiltro;
        return matchBusca && matchStatus;
    });

    const totalFinalizado = encaminhamentos.filter(e => e.status === 'Finalizado').length;
    const totalRascunho = encaminhamentos.filter(e => e.status === 'Rascunho').length;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-1">
                <div>
                    <div className="flex items-center gap-2 text-primary mb-1">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest opacity-60">Interconsultas</span>
                    </div>
                    <h1 className="text-slate-900 dark:text-slate-100 text-3xl font-bold tracking-tight">Encaminhamentos</h1>
                    <p className="text-slate-500 font-medium mt-1">Gestão de referenciamentos clínicos.</p>
                </div>
                <button
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 px-1">
                {[
                    { label: 'Todos', value: encaminhamentos.length, icon: 'folder_open', cor: 'text-primary bg-primary/10' },
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 dark:bg-slate-800/50">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-slate-400">Especialidade</th>
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {filtrados.map((e, i) => (
                                <tr
                                    ...
                                    onClick={() => navigate(`/encaminhamentos/${e.id}`)}
                                >
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className={`size-10 rounded-full flex items-center justify-center text-xs font-bold ${e.pacienteCor || 'bg-primary/10 text-primary'}`}>
                                                {e.pacienteIniciais || '?'}
                                            </div>
                    ...
                    encaminhamentos.length === 0 ? (
                        <div className="py-16 flex flex-col items-center text-center px-6">
                            <div className="size-24 rounded-3xl bg-primary/5 border border-primary/10 flex items-center justify-center mb-5">
                                <span className="material-symbols-outlined text-5xl text-primary/30">send</span>
                            </div>
                            <h3 className="text-lg font-black text-slate-800 dark:text-white mb-2">Nenhum encaminhamento ainda</h3>
                            <p className="text-sm text-slate-400 max-w-sm mx-auto mb-6 leading-relaxed">
                                Encaminhamentos referenciam pacientes para outros profissionais ou especialidades, documentando a indicação clínica de forma segura.
                            </p>
                            <button onClick={() => navigate('/encaminhamentos/novo')} className="px-6 py-3 bg-primary text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all flex items-center gap-2">
            ...
            {confirmDelete && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setConfirmDelete(null)}>
                    <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-sm overflow-hidden" onClick={ev => ev.stopPropagation()}>
                        <div className="p-6 text-center">
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Excluir Encaminhamento?</h3>
                            ...
                            <div className="flex gap-3">
                                ...
                                <button onClick={() => { deleteEncaminhamento(confirmDelete); setConfirmDelete(null); }} className="flex-1 py-2 bg-red-500 text-white rounded-lg text-sm font-bold">Excluir</button>
                            </div>
                        </div>
                    </div>
                </div>
